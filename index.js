   ...
   qrcode.generate(qr, { small: true })
	...
	kev.on('open', () => {
		...
		kev.updatePresence(from, Presence.composing)
					anju = kev.chats.all()
						for (let _ of anju) {
							sendMess(_.jid, `*「 kevbot broad cast 」*\n\nbot on`)
						}
	})
...
*/
           ...
           {
           kev.toggleDisappearingMessages(from, 'halo?', text)
           }
           else if (holai == 'hai')
           {
           kev.sendMessage(from, 'hai juga kak :)', text, {quoted:mek}) 
           }
           else if (holai == 'Hai')
           {
           kev.sendMessage(from, 'hai juga kak :)', text, {quoted:mek}) 
...
break
            ...
            var myMonth = ['januari', 'februari', 'maret', 'april', 'mei', 'juni', 'juli', 'agustus', 'september', 'oktober', 'desember'];
...
datia = await fetchJson(`https://videfikri.com/api/jumlahhuruf/?query=${hf}`)
jumlah = datia.result.jumlah
reply(jumlah)
break
				case 'reverse':
...
try {
datia = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${nk}`, {
  method: 'get'
})
reply(ind.wait)
...
break
				...
				case 'neonimesearch':
					kev.updatePresence(from, Presence.composing)
					ij = body.slice(16) 
					datia = await fetchJson(`https://tobz-api.herokuapp.com/api/neonime?q=${ij}&apikey=${TobzKey}`, {method: 'get'})
					if (!isRegistered) return reply(ind.noregis())
				...
				case 'bacakomik':
				    kev.updatePresence(from, Presence.composing)
					bk = body.slice(12) 
					datia = await fetchJson(`https://tobz-api.herokuapp.com/api/bacakomik?q=${bk}&apikey=${TobzKey}`, {method: 'get'})
					if (!isRegistered) return reply(ind.noregis())
				...
				case 'neonimeongoing':
					kev.updatePresence(from, Presence.composing)
					datia = await fetchJson(`https://tobz-api.herokuapp.com/api/neonimeongoing?apikey=${TobzKey}`, {method: 'get'})
					if (!isRegistered) return reply(ind.noregis())
				...
				case 'anoboyongoing':
					kev.updatePresence(from, Presence.composing)
					datia = await fetchJson(`https://tobz-api.herokuapp.com/api/anoboyongoing?apikey=${TobzKey}`, {method: 'get'})
					if (!isRegistered) return reply(ind.noregis())
				...
				case 'anoboysearch':
					kev.updatePresence(from, Presence.composing)
					kij = body.slice(14) 
					datia = await fetchJson(`https://tobz-api.herokuapp.com/api/anoboy?q=${kij}&apikey=${TobzKey}`, {method: 'get'})
					if (!isRegistered) return reply(ind.noregis())
			    ...
			    case 'ocr':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('kirim gambar dengan caption !ocr')
					}
					break
                case 'ownermenu':
                if (isBanned) return reply(ind.baned())					
...
*/
				...
				if (!isNsfw) return reply(ind.nsfwoff())
					...
					var kongou = require('kongou')
					kongou.get(178513).then((data) => {
                    reply(data);
					kev.sendMessage(from, data, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${body.slice(12)}.pdf` })
					});
					await limitAdd(sender)
					break
				case 'nhentai':
		...
		case 'halloweentext':
            ...
            break
				...
				case 'tomedia':
                                        ...
                                        const buff = await getBuffer(rume.datia.result)
                ...
                reply(`jika kamu menang kamu mendapatkan coin ${ininy}©`)
                ...
                if ( userspilih == computer ) {
                    reply(`Pertandingan Seri!\nkamu mendapatkan ${ininy2}`)
                    addKoinUser(sender, ininy2)
                } else if ( userspilih == 'batu' ) {
                    if( computer == 'gunting' ) {
                        // hasil = 'MENANG';
                        reply(`Kamu memilih Batu dan bot Gunting\nKamu menang!`)
                        addKoinUser(sender, ininy)
                    } else {
...
break
				case 'tebakangka':
				var ininy = Math.ceil(Math.random() * 300000)
				reply(`GAME TEBAK ANGKA \n\nTebak angka dari 1-10. jika menang anda akan medpata kan uang ${ininy}`);
var comp = Math.floor(Math.random() *10) + 1;
console.log(comp);
for(i = 2; i >= 0; i--) {

    //get numbernya dimasukin ke dalem looping
    var player = body.slice(12) 

    if(player == null || player == '') {
        break;
    } else if (player == comp) {
        reply('Selamat tebakan anda benar, angka-nya adalah: ' + comp);
        addKoinUser(sender, ininy)
        break;
    } else if (player < comp) {
        reply('anda kalah yang bnar angka :' + comp)
        break;
    } else if (player > comp) {
        reply('anda kalah yang bnar angka :' + comp)
        break;
...
*/
		    ...
		    iniljawaban = `${unai.result.jawaban}`
					...
					case 'jawab':
					unai = await fetchJson(`https://api.zeks.xyz/api/tebakgambar?apikey=LiaTPHt6xgGSng88ZxT0gpPXyvb`, {method: 'get'})
					var ink = body.slice(7) 
					var iabann = `${unai.result.jawaban}`
					if(iabann == null || player == '') {
        break;
    } else if (ink == iabann) {
        reply('Selamat tebakan anda benar, angka-nya adalah: ' + comp);
        break;
...
case 'ttt':
                ...
                kev.sendMessage(from, text, extendedText, {contextInfo: {"mentionedJid": [penantang, lawan]}}).then(() => {
                    var data = {}
...
*/
		case 'play':   
	          if (!isRegistered) return reply(ind.noregis())
                ...
                reply(ind.download)
                play = body.slice(5)
                anu = await fetchJson(`http://zekais-api.herokuapp.com/ytplay?query=${play}`)
               if (anu.error) return reply(anu.error)
		...
		case 'ytmp4':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('Urlnya mana um?')
					reply(ind.download)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4/2?apikey=LiaTPHt6xgGSng88ZxT0gpPXyvb&url=${body.slice(7)}`)
					if (anu.error) return reply(anu.error)
					tjks = `*Title* : ${sender}\n*Size* : ${anu.result.size}`
					reply('bentar kak') 
					thumb = await getBuffer(anu.result.thumb)
					kev.sendMessage(from, thumb, image, {quoted: mek, caption: tjks})
		...
		case 'ytmp3':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('Urlnya mana um?')
					reply(ind.download)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3/2?apikey=LiaTPHt6xgGSng88ZxT0gpPXyvb&url=${body.slice(7)}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'igvideo':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('Urlnya mana um?')
					anu = await fetchJson(`https://api.zeks.xyz/api/ig?apikey=LiaTPHt6xgGSng88ZxT0gpPXyvb&url=${body.slice(9)}`)
					if (anu.error) return reply(anu.error)
		...
		hem = await fetchJson(`https://api.zeks.xyz/api/tiktok?url=${linktod}&apikey=LiaTPHt6xgGSng88ZxT0gpPXyvb`)
		...
		case 'joox':
				...
				if (!isRegistered) return reply(ind.noregis())
				reply(ind.download)
				datia = await fetchJson(`https://tobz-api.herokuapp.com/api/joox?q=${body.slice(6)}&apikey=${TobzKey}`, {method: 'get'})
			      if (anu.error) return reply(anu.error)
				    infomp3 = `*Lagu Ditemukan!!!*\nJudul : ${datia.result.judul}\nAlbum : ${datia.result.album}\nDipublikasi : ${datia.result.dipublikasi}\n*Filesize* : ${data.filesize}\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM*`
				bufferddd = await getBuffer(datia.result.thumb)
				lagu = await getBuffer(datia.result.mp3)
				kev.sendMessage(from, bufferddd, image, {quoted: mek, caption: infomp3})
				kev.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${datia.result.title}.opus`, quoted: mek})
				break
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isOwner) return reply(ind.ownerb())
				if (args.length < 1) return reply('contoh : !adduang @kev|1')
...
try {
datia = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${hk}`, {
  method: 'get'
})
reply(ind.wait)
n = JSON.parse(JSON.stringify(data));
...
try {
datia = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${jk}`, {
  method: 'get'
})
reply(ind.wait)
n = JSON.parse(JSON.stringify(data));
...
*/				
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
			datia = await fetchJson(`https://tobz-api.herokuapp.com/api/moddroid?q=${body.slice(10)}&apikey=${TobzKey}`)
			hepi = datia.result[0] 
			teks = `*➸ Nama*: ${datia.result[0].title}\n*➸ publisher*: ${hepi.publisher}\n*➸ mod info:* ${hepi.mod_info}\n*➸ size*: ${hepi.size}\n*➸ latest version*: ${hepi.latest_version}\n*➸ genre*: ${hepi.genre}\n*➸ link:* ${hepi.link}\n*➸ download*: ${hepi.download}`
			...
			case 'happymod':
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
			datia = await fetchJson(`https://tobz-api.herokuapp.com/api/happymod?q=${body.slice(10)}&apikey=${TobzKey}`)
			hupo = datia.result[0] 
			...
			break
					...
					if (!isRegistered) return reply(ind.noregis())
					kev.updatePresence(from, Presence.composing) 
				const datia = await fetchJson(`http://zekais-api.herokuapp.com/pinterest?query=${body.slice(11)}`, {method: 'get'})
				reply('[❗] LOADING!')
				n = datia.result
				nimek =  n[Math.floor(Math.random() * n.length)];
				pok = await getBuffer(nimek)
...
break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    kev.updatePresence(from, Presence.composing) 
					datia = await fetchJson(`https://docs-jojo.herokuapp.com/api/infohoax`, {method: 'get'})
					teks = '♡───────────♡\n'
					for (let i of datia.result) {
						teks += `*➸ Gambar* : ${i.image}\n*➸ Title* : ${i.title}\n*➸ link* : ${i.link}\n*➸ tag* : ${i.tag}\n♡───────────♡\n`
					}
					...
					const nga =[`${pertnyan} - ${pertnyan1}`,`${pertnyan} * ${pertnyan1}`]
				    const tau = nga[Math.floor(Math.random() * nga.length)]
				    result = tau;
					kev.sendMessage(from, tau, text) 
					kev.sendMessage(from, result, text) 
					}
					if (args[0] == 'medium') {
...
nama = body.slice(7) 
try {
datia = await fetchJson(`https://api.terhambar.com/ninja?nama=${nama}`)
...
break
  ...
  tels = body.slice(6)
  try {
  datia = await fetchJson(`https://tobz-api.herokuapp.com/api/kbbi?kata=${tels}&apikey=${TobzKey}`)
...
*/
				...
				if (!isOwner) return reply(ind.ownerb())
				in1i = body.slice(5) 
				anu = await fetchJson(`${in1i}`)
					buffer = await getBuffer(anu.result)
					kev.sendMessage(from, buffer, image, {caption: 'Nih kapten', quoted: mek})
					if (err) return reply(`Err: ${err}`)
					break
			   case 'fetch':
			if (!isOwner) return reply(ind.ownerb())
			if (err) return reply(`Err: ${err}`)
			datia = await fetchJson(`${body.slice(7)}`)
			reply(datia) 
			break
				case 'unban':
					if (!isOwner) return reply(ind.ownerb())
